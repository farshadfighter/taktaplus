...
- **فاز ۴ (این نسخه)** — توزیع آفلاین سیگنیچر و فرم‌ور: منبع FTP (پیش‌فرض
...
backend/app/
  ...
  domains/     یک پکیج به ازای هر بخش:
               ...
               (منبع FTP، مدیریت بسته‌ها، پوش سیگنیچر/فرم‌ور)، self_update
               (تایید بسته آپدیت امضاشده)، diagnostics (خروجی بسته تشخیصی)
  api/v1/      تجمیع روتر، دیپندنسی‌های auth/RBAC
  workers/     Celery app + task های heartbeat لایسنس، بکاپ دیتابیس داخلی،
               بکاپ زمان‌بندی‌شده دستگاه‌ها، poll دوره‌ای SNMP، و همگام‌سازی
               شبانه FTP؛ snmp_trap_receiver.py و ftp_relay_server.py هم
               اینجان ولی هرکدوم یک process کاملاً جدا و مستقل از
...
frontend/src/
  ...
  modules/     auth (لاگین)، dashboard، devices (لیست/افزودن/تست اتصال)،
               ...
               به‌ازای هر دستگاه)، reports (گزارش وضعیت فلیت)، license
...
- **محدودیت عملیاتی**: تغییر تنظیمات SNMP یک دستگاه نیاز به ری‌استارت
  process جدای `snmp-trap-receiver` داره (بدون hot-reload).
- Trap Receiver به `asyncore` (ماژول deprecated پایتون که در ۳.۱۲ حذف
  می‌شه) وابسته‌ست؛ به همین دلیل Dockerfile روی Python 3.11 پین شده -
  قبل از ارتقای نسخه پایتون، این وابستگی باید بازبینی بشه.
...
- `pyftpdlib` هم مثل pysnmp به `asyncore`/`asynchat` وابسته‌ست — همون
  محدودیت پین Python 3.11 رو تقویت می‌کنه.

...
- سرویس پیامکی کاملاً توسط مشتری تنظیم می‌شه — برخلاف منبع FTP فاز ۴ هیچ
  ...
  نشده بود.
- **محدودیت عملیاتی**: مثل SNMP trap receiver و FTP relay، اضافه/حذف/ویرایش
  یک `RadiusClient` (NAS) نیاز به ری‌استارت process جدای `radius-server`
  داره (بدون hot-reload).

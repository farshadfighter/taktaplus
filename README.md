...
- **فاز ۰** — زیرساخت پایه: احراز هویت/RBAC، Audit Log، سیستم
  ...
  بسته تشخیصی.
- **فاز ۱ (این نسخه)** — مدیریت دستگاه‌ها: افزودن/حذف/لیست FortiGate و
...
backend/app/
  ...
  domains/     یک پکیج به ازای هر بخش:
               ...
               licensing (فعال‌سازی/heartbeat/enforcement)، self_update (تایید بسته
               آپدیت امضاشده)، diagnostics (خروجی بسته تشخیصی)
  api/v1/      تجمیع روتر، دیپندنسی‌های auth/RBAC
  workers/     Celery app + task های heartbeat لایسنس و بکاپ خودکار دیتابیس
  alembic/     migration ها

frontend/src/
  app/         روتر
  components/  layout (RTL shell, sidebar)
  modules/     auth (لاگین)، dashboard، license (بنر وضعیت + صفحه فعال‌سازی)
  services/    کلاینت API
...
## نکته مهم فاز ۱: احراز هویت FortiWeb نیاز به تایید روی دستگاه واقعی دارد

درایور FortiGate (`backend/app/domains/devices/drivers/fortigate.py`) با توکن
API استاندارد Fortinet ساخته و تست شده و قابل اعتماد است. درایور FortiWeb
(`drivers/fortiweb.py`) چون مستندات رسمی و پایداری در حد FortiGate برایش پیدا
نشد، با فرض ورود session-based کلاسیک خانواده FortiOS (`POST /logincheck` +
کوکی/هدر CSRF) پیاده شده و فقط با یک سرور جعلی تست شده - **باید روی FortiWeb
واقعی که در دسترس دارید تست بشه**. اگه «تست اتصال» روی FortiWeb واقعی fail
داد، مسیر endpoint/فرمت لاگین را در همان فایل اصلاح کنید؛ چون فقط پشت اینترفیس
`FortinetDriver` قرار داره، این تغییر به بقیه‌ی کد سرایت نمی‌کنه.

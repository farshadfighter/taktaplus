...
- **فاز ۵ (این نسخه)** — احراز هویت دو مرحله‌ای پیامکی از طریق RADIUS: یک
  ...
  پیکربندی می‌شود (کاوه‌نگار و sms.ir تأییدشده + یک مسیر HTTP سفارشی برای
...
- سرویس پیامکی کاملاً توسط مشتری تنظیم می‌شه — برخلاف منبع FTP فاز ۴ هیچ
  ...
  تایید شده؛ sms.ir هم با خوندن سورس SDK رسمی‌اش (چون سایت مستندات
  apidocs.sms.ir توسط پراکسی این محیط بلاک بود) تایید شد -
  `POST https://api.sms.ir/v1/send/bulk` با هدر `X-API-KEY` و بدنه
  `{lineNumber, messageText, mobiles, sendDateTime}`؛ نکته مهم: sms.ir
  همیشه HTTP 200 برمی‌گردونه، موفقیت/خطا فقط از فیلد `status` داخل پاسخ
  (۱ یعنی موفق) قابل تشخیصه، نه کد HTTP. برای هر سرویس دیگه (مثلاً
  ملی‌پیامک) یک مسیر HTTP سفارشی قابل‌تنظیم (`generic_http`) هست چون
  شکل دقیق API اون سرویس‌ها تایید نشده بود.
...
- **افزودن کاربر از روی دستگاه، نه از صفر**: به‌جای تایپ دستی نام کاربری،
  صفحه «کاربران احراز هویت پیامکی» می‌تونه لیست کاربران ادمین و VPN محلی
  واقعی روی FortiGate رو بگیره (`GET /devices/{id}/local-users`، فعلاً فقط
  FortiGate) و عملگر از بینشون انتخاب کنه - اگه شماره موبایل قبلاً روی
  خودِ دستگاه (ویژگی بومی SMS 2FA خودِ FortiGate) تنظیم شده باشه، خودکار
  پر می‌شه. **رمز عبور رو نمی‌شه از دستگاه خوند** (فورتی‌نت هیچ‌وقت رمز
  موجود رو برنمی‌گردونه) - پس عملگر باید رمز رو دستی وارد کنه و با رمز
  واقعی روی دستگاه هماهنگ نگه داره؛ این یک محدودیت ذاتی معماریه (RADIUS
  به‌عنوان بک‌اند اصلی احراز هویت)، نه چیزی که taktaplus بتونه دورش بزنه.
  با یک سرور HTTPS واقعی (جایگزین FortiGate) و یک مرورگر واقعی تست شده،
  نه فقط mock.

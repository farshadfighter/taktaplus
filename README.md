...
- **فاز ۲** — بکاپ/ریستور: بکاپ دستی و زمان‌بندی‌شده (شبانه) با
  ...
  با `force`).
- **فاز ۳ (این نسخه)** — مانیتورینگ SNMP (v2c) + هشداردهی + گزارش Compliance:
...
backend/app/
  ...
  domains/     یک پکیج به ازای هر بخش:
               ...
               ایمیل/تلگرام)، reporting (گزارش وضعیت فلیت)، self_update (تایید
               بسته آپدیت امضاشده)، diagnostics (خروجی بسته تشخیصی)
  api/v1/      تجمیع روتر، دیپندنسی‌های auth/RBAC
  workers/     Celery app + task های heartbeat لایسنس، بکاپ دیتابیس داخلی،
               بکاپ زمان‌بندی‌شده دستگاه‌ها، و poll دوره‌ای SNMP؛
               snmp_trap_receiver.py هم اینجاست ولی یک process کاملاً جدا
               و مستقل از FastAPI/Celery است (چون socket UDP خودش رو باز
               نگه می‌داره)
...
frontend/src/
  ...
  modules/     auth (لاگین)، dashboard، devices (لیست/افزودن/تست اتصال)،
               ...
               دستگاه)، reports (گزارش وضعیت فلیت)، license (بنر وضعیت +
               صفحه فعال‌سازی)
  services/    کلاینت API
...
docs/          مستندات معماری: قرارداد API لایسنسینگ، مدیریت کلید رمزنگاری،
               معماری بکاپ/ریستور، معماری مانیتورینگ SNMP
```
